...
export function signWorkshopToken(): string {
  const payload = { role: "workshop", exp: Date.now() + 7 * 24 * 60 * 60 * 1000 };
...
/**
 * Wenn RABBIT_WORKSHOP_PASSWORD gesetzt ist: Bearer-Token nötig.
 * Ohne Passwort: öffentlich (nur für Entwicklung empfohlen).
 */
export function requireWorkshopAuth(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!tok || !verifyWorkshopToken(tok)) {
